...
import { ExistsCard } from "@/components/dashboard/ExistsCard";
import { ProductTopOptions } from "@/declarations";
import { ISalesDashboard } from "@/declarations/model/dashboard";
...
export default function DashboardPage() {
  const [salesDashboard, setSalesDashboard] = React.useState<
    ISalesDashboard | undefined
  >();

  const [salesProductOptions, setSalesProductOptions] = React.useState<ProductTopOptions>({
    top: 5,
    year: 2024,
    month: 7,
    order: "DESC"
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (response?.success) {
        setSalesDashboard(response.content);
      }
    })();
  }, []);
  ...
  const refreshProductSales = async () => {
    ...
    if( response?.success ){
      setSalesDashboard({
        ...salesDashboard,
        topProductByYear: response.content
      });
    }
  }
